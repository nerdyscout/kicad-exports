...
RUN pip3 install kikit==0.3
RUN pip3 install kibom==1.7.1 
RUN pip3 install kicost==1.1.4
RUN apt-get install -y python3-cairo && pip3 install pcbdraw==0.2.1 
...
COPY /opt/ibom /opt/InteractiveHtmlBom
## kiplot
RUN apt-get install -y python3-yaml
COPY submodules/kiplot /opt/kiplot
COPY opt/kiplot /opt/kiplot
...
COPY functions.sh .
COPY kicad-exports.sh /root/entrypoint.sh
